...
Migration 014 adds the authenticated `SECURITY INVOKER` RPC `save_zepbound_daily_log` and hardens the symptom and check-in coordination triggers; applying the migration does not mutate existing rows. The UI calls that RPC exactly once and performs no direct symptom/check-in writes.
...
The RPC takes its blocking owner/date advisory lock before touching rows. The `BEFORE INSERT OR UPDATE OR DELETE` triggers protecting all direct legacy symptom and check-in writes use only `pg_try_advisory_xact_lock`, because an UPDATE or DELETE statement may already hold a tuple lock when its row trigger runs. This includes symptom severity/notes-only updates and symptom deletes, not just owner/date/type changes. If the owner/date key is busy, either trigger immediately raises retryable PostgreSQL `serialization_failure` (`40001`); the legacy transaction aborts and releases its tuple lock instead of waiting and deadlocking with the RPC. A cached legacy write therefore cannot silently interleave with a complete save or leave mixed symptom/check-in state. Key-moving updates try old/new advisory keys in deterministic numeric order and abort if either is unavailable. Inserts, deletes, and same-key updates try one key. Trigger calls made by the lock-owning RPC are reentrant and succeed. After locking, symptom inserts and updates retain the existing `None`-versus-real-symptom exclusivity check; deletes need no exclusivity check.
...
Migration contract tests cover explicit transaction boundaries, no migration-time data DML, complete validation before mutation, shared lock, fail-fast `40001` rejection for all direct symptom/check-in DML with no blocking lock in either trigger, deterministic key-moving locks, None exclusivity, replacement/upsert/delete behavior, injection association, invoker security, and grants. Hook/UI tests cover one complete RPC call, one action/save, hydration and field preservation, empty symptoms, unanswered check-in, None, custom symptoms, clear-answer behavior, failed-draft retention, date switching, shot separation, and read-only Health history.
