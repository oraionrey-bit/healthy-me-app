...
4. Schedule a brief maintenance window if needed. The ordinary unique-index build intentionally runs in the same migration transaction as deduplication, preventing a concurrent write from recreating a duplicate between cleanup and index enforcement; it can briefly block writes to the symptom table.
